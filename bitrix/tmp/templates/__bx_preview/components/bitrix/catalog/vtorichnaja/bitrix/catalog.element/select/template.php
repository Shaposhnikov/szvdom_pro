...
{
?>
<div class="bx_item_title"><h1 style="margin-bottom: 30px;"><span><?
	echo (
...
if (($APPLICATION->GetCurPage() == $arResult['DETAIL_PAGE_URL']) || ($APPLICATION->GetCurPage() == $arResult['DETAIL_PAGE_URL']."tseny-na-kvartiry/") || ($APPLICATION->GetCurPage() == $arResult['DETAIL_PAGE_URL']."planirovki/")){
	...
	<script type="text/javascript">
		...
		function init () {
            ...
            geometry: {
                type: "Point",
                coordinates: [<?=$mapLabel["Y"]?>, <?=$mapLabel["X"]?>]
	            },
	            properties: {
	                iconContent: '<?=$mapLabel["NAME"]?>',
	            }
	        }, {
            preset: 'islands#blackStretchyIcon',
            draggable: false
        	});
        	    headMap.geoObjects.add(myGeoObject);
		}
	</script>
    <?
    $imageArray = array();
	...
	<div class="objectHead">
		<div class="objectHeadFotoGroup">
            <div class="bigImageFotoGroup">
                <img width="400" height="300" src="<?=$imageArray[0]["SRC"];?>" title="<?=$imageArray[0]["SRC"];?>" alt="<?=$imageArray[0]["SRC"];?>" />
            </div>
            <div class="littleImagesFotoGroup">
                <?foreach($imageArray as $value){?>
                    <img src="<?=$value["SRC"];?>" onclick="$('.bigImageFotoGroup img').attr('src','<?=$value["SRC"];?>');" alt="<?=$value["DESCRIPTION"];?>" title="<?=$value["DESCRIPTION"];?>" width="60" height="40" />
                <?}?>
            </div>
		</div>
		<div class="objectHeadMapPlace">
			<div id="objectHeadMap" style="width: 480px;height: 400px;"></div>
		</div>
	</div>
<?}
if ($APPLICATION->GetCurPage() == $arResult['DETAIL_PAGE_URL']){?>
	<div class="objectProperties">
		<div class="objectPropertiesRightSide">
			<table>
				<tbody>
					...
					<tr>
						<td>Регион</td>
						<td><?if (!empty($arResult["PROPERTIES"]["REGION"]["VALUE"])){
							$str = "";
							foreach ($arResult["PROPERTIES"]["REGION"]["VALUE"] as $key => $value) {
								if ($key != (count($arResult["PROPERTIES"]["REGION"]["VALUE"]) - 1)){
									$str .= $value.", ";
		...
		<div class="objectPropertiesLeftSide">
            <p>Преимущества</p>
            <div class="objectPropertiesLeftSideInner">
                <?=htmlspecialcharsBack($arResult["PROPERTIES"]["DESCRIPTION_ADVANTAGE"]["VALUE"]["TEXT"]);?>
            </div>
...
/*    		echo "<pre>";
    ...
    $APPLICATION->IncludeComponent(
		...
		array(
			...
			"XML_ID" => $arResult["PROPERTIES"]["SECOND_ID"]["VALUE"],
			"REDIRECT_URL" => $arResult['DETAIL_PAGE_URL']."tseny-na-kvartiry/"

		),
		false
		);
}elseif (($APPLICATION->GetCurPage() == $arResult['DETAIL_PAGE_URL']."tseny-na-kvartiry/") || ($APPLICATION->GetCurPage() == $arResult['DETAIL_PAGE_URL']."planirovki/")) {
    if ($APPLICATION->GetCurPage() == $arResult['DETAIL_PAGE_URL']."tseny-na-kvartiry/"){
        $flag = "price";
    }elseif($APPLICATION->GetCurPage() == $arResult['DETAIL_PAGE_URL']."planirovki/"){
        $flag = "plan";
    }
...
}elseif ($APPLICATION->GetCurPage() == $arResult['DETAIL_PAGE_URL']."pohozhie-obekty/"){

    foreach ($simalar as $value){
        if ($value["title"] != $blocks[0]["title"]){
            $mapSimalarLabel[] = array( "X" => (float)$value["longitude"], "Y" => (float)$value["latitude"], "NAME" => (string)$value["title"]);
        }
    }
/*    echo "<pre>";
    var_dump( $mapSimalarLabel);
    echo "</pre>";*/
	?>
	<script type="text/javascript">
		var myMap;
		...
		function init () {
			var myMap = new ymaps.Map("map", {
	            center: [<?=$mapSimalarLabel[0]["Y"]?>, <?=$mapSimalarLabel[0]["X"]?>],
	            zoom: 8,
	            controls: ['smallMapDefaultSet']
	        }),
...
<div class="specialAndBuying">
    <div class="specialBlockElement">
        <div class="specialBlockElementTitle">Акции и скидки</div>
        <ul>
        <?foreach($arResult["PROPERTIES"]["SPECIAL_OFFER"]["VALUE"] as $value){?>
            <li> - <?=$value;?></li>
       <? }?>
        </ul>
        <button>Жмакай кнопку</button>
    </div>
    <div  class="buyingBlockElement">
        <div class="buyingBlockElementTitle">Способы покупки</div>
        <?foreach($arResult["PROPERTIES"]["EX_EXTRAS"]["VALUE"]as $value){?>
            <p> - <?=$value;?></p>
        <? }?>
        <a href="javascript: void(0);">Узнать подробности</a>
    </div>
</div>
<div class="mortgageAndCalculator" style="height: 100px;width:1000px;border: 1px solid;">
    <p style="font-size: 20px; text-align: center;">Два блока("зачет жилья" и "ипотека") будут тут. Обещаю.</p>
</div>
